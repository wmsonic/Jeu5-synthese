...
public class BiomeEtatInexplorerMouiller : BiomesEtatsBase
{
    ParticleSystem _particleSystem;
    ParticleSystem _particleSystemSubEmitter;
    bool _canAnimate = true;
    public override void InitEtat(BiomesEtatsManager biome){
        //display material inexplorerMouiller
    ...
    public override void TriggerEnterEtat(BiomesEtatsManager biome, Collider obj){
        // Debug.Log("triggered!!");
        if(obj.CompareTag("ChampForce_player")){ // doit changer bloc pour state explorer mouiller et bonne texture (_5) & slowdown animation vue que sous l'eau
            if(_canAnimate){
                _canAnimate = false;
                biome.StartCoroutine(Anime(biome));
            }
        } //Maybe else if les ennemis walk dessus ils les brisent et donc rip l'item dessus ?
    }
    ...
    private IEnumerator Anime(BiomesEtatsManager biome){
        ...
        Vector3 targetScale = biomeTransform.localScale * Random.Range(.5f,1.5f); // Définie un scale aléatoire entre deux valeur cube, qui sera la grosseur vers laquelle on veut transformer le cube
        Vector3 originalPos = biomeTransform.position; // Récupère la position original du cube
        Vector3 targetPos = new Vector3(biomeTransform.position.x, biomeTransform.position.y + Random.Range(.5f,1.2f), biomeTransform.position.z); // Définie une position qui sera la position vers laquelle on veut déplacer le cube (Dans se cas : hauteur aléatoire entre deux valeur cube)
        yield return AnimateBetweenScalesAndPosition(biomeTransform,originalScale, targetScale, originalPos, targetPos, 0.8f); // début : Appel d'une coroutine pour animer mon cube vers mes cibles de grosseur et position depuis sa grosseur et position d'origine
        biome.GetComponent<Renderer>().material = biome.biomeMaterielExplorerMouiller; // change le material du cube à son material explorer
        yield return new WaitForSeconds(.1f); // Attend un peu
        _particleSystem.Play(); // Fait jouer le système de particule attacher au cube
        _particleSystem.GetComponent<ParticleSystemRenderer>().material = biome.biomeMaterielExplorerMouiller; //Set le matériel de mes particules au même matériel que celui du cube
        yield return AnimateBetweenScalesAndPosition(biomeTransform,targetScale, originalScale, targetPos, originalPos, 0.4f); // fin : Appel d'une coroutine pour animer mon cube vers sa grosseur et position d'origine depuis mes cibles de grosseur et position
        biome.ChangerEtat(biome._explorerMouiller); // change l'état de mon cube pour qu'il soit à l'état explorer
    }
